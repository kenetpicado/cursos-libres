<div class="card">
    ...
    <x-modal2 label="Agregar Alumno al Grupo">
        ...
        <input class="form-control" type="search" placeholder="Buscar alumno" wire:model="search">
        <x-table>
            @slot('header')
                <th>Carnet</th>
                <th>Nombre</th>
                <th>
                    <span class="float-end">Acción</span>
                </th>
            @endslot
            @forelse ($this->results as $result)
                <tr>
                    <td>{{ $result->carnet }}</td>
                    <td>{{ $result->nombre }}</td>
                    <td><button class="btn btn-sm btn-primary float-end"
                            wire:click="$emit('inscribir', {{ $result->id }})">Agregar</button></td>
                </tr>
            @empty
                <tr>
                    <td colspan="3" class="text-center">No hay coincidencias</td>
                </tr>
            @endforelse
        </x-table>
    </x-modal2>
